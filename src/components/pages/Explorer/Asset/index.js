...
import { processAsset, imageStringToCloudflare, randomHSL, validUrl } from "@/utils/index"
...
const Asset = ({ fingerprint }) => {
  ...
  const metadataNftTransformed = assetInfo?.metadataNft && Object.keys(assetInfo?.metadataNft).map((key) => {
  ...
  return (
    <div className="ray__block pt-3">
      ...
      )}
      {
        (!loading && found) && (
          <div>
            <div className={`${style.preview} ${isLight ? style.previewLight : ''}`}>
              <div
              ...
              <div className={style.previewInner}>
                <div className="px-5">
                  <h1 className="mb-1">
                    <span className={style.title}>{assetInfo.metadataNft?.name || assetInfo.assetName}</span>
                  </h1>
                  <p className="text-muted mb-3">Quantity: <strong>{assetInfo.quantity || 0}</strong> — {fingerprint}</p>
                </div>
                  ...
                  <div>
                    ...
                    <div className={style.image}>
                      <img
                        ...
                        src={imageStringToCloudflare(assetInfo.metadataNft?.image)}
                      />
                      {loadingImg && (
                          ...
                          <div className="mt-3">
                            ...
                            <Tooltip title={(
                              <div className="text-center">
                                If you see Ray Diamond on the following link, you don't need to take any action, just surf NFTs!
                              </div>
                            )}>
                              <a href="https://cloudflare-ipfs.com/ipfs/QmaYWWWmrUJkWiKAaHRiYwLaSCNGT8he4ZpuQd5TddvRVJ" target="_blank" rel="noopener noreferrer">Pass the captcha!</a>
                            </Tooltip>
                          </div>
                        </div>
                      )}
                          ...
                          if (stopWords.includes(key)) {
                            ...
                            return (
                              <span style={{ color: randomHSL() }}>
                                <span className="text-capitalize">{key}</span>: <a href={value} target="_blank" rel="noopener noreferrer">Link</a>{metadataNftTransformed.length === index + 1 ? '' : ', '}
                              </span>
                            )
                          } else if (typeof value === 'string' || typeof value === 'number') {
                            return (
                              <span style={{ color: randomHSL() }}>
                                <span className="text-capitalize text-break">{key}</span>: {value}{metadataNftTransformed.length === index + 1 ? '' : ', '}
                              </span>
                            )
                          }
                          return (
                            <span style={{ color: randomHSL() }}>
                              <span className="text-capitalize text-break">{key}</span>: [...]{metadataNftTransformed.length === index + 1 ? '' : ', '}
                            </span>
                          )
                        })}
                      </div>
            ...
            <div>
              <div className="text-center mb-4">
                ...
                <Link to={`/explorer?policyID=${assetInfo.policyId}`} className="link--dashed">
                  {assetInfo.policyId}
                </Link>
              </div>
              <div className="mb-4">
                <Helmet>
                  <meta property="og:title" content={`${assetInfo.metadataNft?.name || assetInfo.assetName} — Cardano NFT Token`} />
                  <meta property="og:image" content={imageStringToCloudflare(assetInfo.metadataNft?.image)} />
                </Helmet>
                <InlineShareButtons
                  config={{
                    enabled: true,
