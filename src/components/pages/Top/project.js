...
const Top = ({ project, rank }) => {
  ...
  const cutLink = (link) => link.replaceAll('https://www.', '').replaceAll('https://', '')
        ...
        <div className={style.projectInfo}>
          <div className={`${style.projectVote} mb-2`}>
            <div>
              <span className={style.projectVoteLabel}>
                ...
                <Popover content={<QRCode value={project.voteAddress} size="50" />}>
                  <span className="link">QR Code</span>
                </Popover>
              </span>{' '}
              <CopyToClipboard text={project.voteAddress} onCopy={onCopy}>
                <Tooltip title="Copy to clipboard">
                  <span className={`${style.projectVoteAddress} cursor-pointer`}>
          ...
          <div className="mb-2">
            <span className="me-2">
              ...
              <a href={project.url} target="_blank" rel="noopener noreferrer">{cutLink(project.url)}</a>{' '}
            </span>
            <span className="me-2">
              <span className="ray__icon ray__icon--inline">
                <SVGTwitter />
              </span>{' '}
              <a href={project.twitter} target="_blank" rel="noopener noreferrer">{cutLink(project.twitter)}</a>
            </span>
          </div>
          <div>
            <span className={`${style.projectLabel} me-2`}>Explore Policies ({project.policies.length})</span>
